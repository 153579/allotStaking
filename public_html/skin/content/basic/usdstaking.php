<?php
	...
	if($master['mc_dnb2'] == "off" && $member['mb_id'] != "admin"){
		alert("지금은 참여 기간이 아닙니다." , "http://allot-staking.com/");
	}

	$sql = "select * from g5_product where prd_del_yn = 'N' and prd_type = 'DNB2'";
	$rtn = sql_query($sql);
...
<div id="dnbppwrap">
    <div class="dnbplus_top">
        ...
        <p data-i18n="ALT:4-1" style="color:red"></p><br>
        <form action="" style="width:100%; text-align:center">
            <input type="checkbox" name="buy_check" id="buy_check" style="margin:0 0 8px 0">&nbsp;<label for="" data-i18n="ALT:J">위 약정에
                동의하십니까?</label>
        </form>
    </div>

    <div class="dnbplus_top" style="margin-top:25px; padding:0 20px; word-break:break-all">
        <p style="margin-top:15px; color:#0080f3;" data-i18n="ALT:kkk">◆ 예치하신 USD로 환산된 ALT수량은 스테이킹 마지막날 출금이 가능하며 리워드는 매일 오전 정산 지급되고 출금 또한 가능합니다. ◆</p>
    </div>
			...
			?>
                <div class="pack_cont">
                    <div class="pack_cont_text" style="padding:15px">
                        ...
                        <ul class="ct_plus">
                            <li class="mg_plus1"><?php echo number_format($row['prd_name']); ?> $</li>
                            <li class="mg_plus"><?php echo $row['prd_percent']; ?>%</li>
                            <li class="mg_plus"><?php echo $row['prd_day_count']; ?>DAY</li>
                            <li class="mg_plus"><?php echo number_format($row['prd_price']); ?> USD</li>
                            <br>
                            <li class="mg_plus">REWARD ALL : <?=$REWARD_ALL ?> USD</li>
                            <li class="mg_plus">REWARD DAY : <?=$REWARD_DAY ?> USD</li>
                        </ul>
                    </div>
                    <button class="pack_btn" type="button"
                        onclick="order_click('<?php echo $row['prd_id']; ?>');" data-i18n="ALT:K">구매하기</button>
                </div>
                <?php } ?>
            </div>
        </div>
    ...
    <div class="tx_box">
        <div id="pack_popup">
            <h3><img src="<?php echo G5_IMG_URL?>/ic_daily@3x.png">&nbsp; <span class="prd_name"><?php echo number_format($my_rtn['prd_price']); ?></span> PLAN</h3>
            <div id="pack_popup_top">
                <p class="p_nth1">
                    ...
                    <button class="bt_copy bt_margin_top" onclick="copyToClipboard('text1')" data-i18n="ALT:M">주소복사</button><br>
                    <p class="p_nth2 bold_text"><span data-i18n="ALT:N">입금하실 ALT :</span> <span class="ether_price"><?php echo number_format($prd_price); ?></span>&nbsp;ALT </p>
                ...
                <form id="popup_form">
                    ...
                    <div id="popup_btn_wrap">
                        <button type="button" class="popup_btn popclose" onclick="order_cancel('<?php echo $my_rtn['ord_id']; ?>' , 'dnbplusplus');" data-i18n="ALT:P">취소</button>
                        <button type="button" class="popup_btn ok_order" onclick="txid_send('<?php echo $my_rtn['ord_id']; ?>');" data-i18n="ALT:Q">확인</button>
                    </div>
                </form>
            </div>
        </div>
